...
cd backend
npm install
cp .env.example .env      # прописать свой GEMINI_API_KEY
...
| `threshold` | `number` | `0` | Минимальное cosine similarity (0–1) |
...
2. **Cosine similarity** — попарное сравнение всех векторов. Результат — таблица «Топ похожих пар», отсортированная по убыванию. **Это основная и точная метрика.**

3. **Semantic map** — embedding-векторы сжимаются до 2D через PCA или UMAP и отображаются на scatter plot. Используется **только как вспомогательная визуализация**. Расстояния на карте могут искажаться и не являются точной метрикой. Точные значения — в таблице справа.
...
- CSV upload, база данных, авторизация, pgvector — не реализованы (выходят за рамки текущего ТЗ).
